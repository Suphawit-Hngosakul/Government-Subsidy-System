...
ส่วนของธนธัชครอบคลุม component ต่อไปนี้:

| Component | Responsibility |
|-----------|----------------|
| Orchestrator Handler | เปิด HTTP endpoints สำหรับ internal orchestration, decision และ SSE |
| Orchestrator Service | ควบคุม flow หลัก, เรียก provider แบบ parallel, รวมผล และ publish status events |
| Decision Engine V1 | ประเมิน eligibility rules จากข้อมูล DOPA, SSO และ KTB |
| DOPA Adapter | แปลงการเรียกตรวจสอบทะเบียนราษฎรให้เป็น `DOPAResult` |
| SSO Adapter | แปลงการเรียกตรวจสอบประกันสังคมให้เป็น `SSOResult` |
| KTB Adapter | แปลงการเรียกตรวจสอบการเงิน/บัญชีให้เป็น `KTBResult` |

ใน Version 1 adapter ทั้ง 3 ตัวยังเป็น mock adapter อยู่ใน package `Backend/adapter` เพื่อให้เปลี่ยนเป็น HTTP adapter จริงได้ภายหลังโดยไม่กระทบ handler หรือ decision logic
...
| Adapter | Future upstream endpoint | Current method |
|---------|--------------------------|----------------|
| DOPA Adapter | `GET /api/v1/dopa/verify/:nationalId` และ `GET /api/v1/dopa/card-status/:nationalId` | `Verify(ctx, nationalId)` |
| SSO Adapter | `GET /api/v1/sso/status/:nationalId` และ `GET /api/v1/sso/contribution/:nationalId` | `Status(ctx, nationalId)` |
| KTB Adapter | `GET /api/v1/ktb/financial-check/:nationalId` และ `GET /api/v1/ktb/account-status/:nationalId` | `FinancialCheck(ctx, nationalId)` |
